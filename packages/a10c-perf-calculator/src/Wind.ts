export interface IWind {
  ...
  longitudinalDirection:
    | WindDirections.Head
    | WindDirections.Tail
    | WindDirections.Neutral;
  lateralDirection:
    | WindDirections.CrossLeft
    | WindDirections.CrossRight
    | WindDirections.Neutral;
}
...
export class Wind {
  ...
  RelativeHeadwind(course: number): number {
    const [headOrTailWind, headOrTail] = this.HeadOrTailwind(course);
    if (headOrTail == WindDirections.Head) {
      return headOrTailWind;
    } else {
      return -headOrTailWind;
    }
  }
  ...
  Winds(course: number): IWind {
    ...
    return {
      front: headOrTailWind,
      cross: crossWind,
      longitudinalDirection: direction,
      lateralDirection: crossDirection,
    };
  }
  ...
  HeadOrTailwind(
    ...
    number,
    WindDirections.Head | WindDirections.Tail | WindDirections.Neutral,
  ] {
    const [angle, headOrTail] = this.getAngle(course);
    const headOrTailWind = Math.abs(
  ...
  private getAngle(
    ...
    let headtailDirection:
      | WindDirections.Head
      | WindDirections.Tail
      | WindDirections.Neutral = WindDirections.Neutral;
    let crossDirection:
      | WindDirections.CrossLeft
      | WindDirections.CrossRight
      | WindDirections.Neutral = WindDirections.Neutral;
