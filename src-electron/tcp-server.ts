  ...
  if (detectUpdate(message)) {
    ...
    while (message.length - offset > 0) {
      ...
      if (startAddress === 0x1110) {
        const antiSkid = decodeValue(dcsbiosData, 0x1110, 0x0080, 7)
          ? 'ON'
          : 'OFF';
        BrowserWindow.getAllWindows().forEach((win) => {
      ...
      CDU_LINES.forEach((line, idx) => {
        if (startAddress === line.address) {
          const cduLineValue = decodeString(
            dcsbiosData,
            line.address,
            line.length,
          );
    ...
    BrowserWindow.getAllWindows().forEach((win) => {
      win.webContents.send(
        'dcsbios-error',
        `Received invalid message: ${bytesToHex(message)}`,
      );
    });
  }
});
...
function decodeValue(
  ...
  if (startAddress < 0 || startAddress + 2 > buffer.length) {
    throw new RangeError(
      `Index ${startAddress} is out of range. Buffer length: ${buffer.length}`,
    );
  }
