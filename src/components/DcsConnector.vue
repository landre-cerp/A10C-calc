<template>
  <div>
    ...
    <div><strong>Anti Skid:</strong> {{ antiSkid }}</div>
    <div><strong>Anti Skid Caution:</strong> {{ clb1 }}</div>
    <div>
...
onMounted(() => {
  if (isElectron) {
    ...
    status_message.value =
      'Electron features are not available in the web version.';
    message.value =
      'Connect to DCS is only available in the desktop (Electron) app.';
  }
});
</script>
